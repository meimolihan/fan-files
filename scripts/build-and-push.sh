...
#   release.yml -> amd64/arm64 二进制创建 GitHub Release（版本跟随 version/version.go）
...
info "写入发版备注 RELEASE_NOTES.md"
{
  ...
  printf 'docker pull mobufan/fan-files:latest\n'
  printf 'docker pull mobufan/fan-files:%s\n' "${TAG}"
  printf '\n'
  printf 'docker pull ghcr.io/meimolihan/fan-files:latest\n'
  printf 'docker pull ghcr.io/meimolihan/fan-files:%s\n' "${TAG}"
} > RELEASE_NOTES.md
...
echo -e "${gl_bai}远程安装命令： ${gl_lv}curl -fsSL https://raw.githubusercontent.com/meimolihan/fan-files/main/scripts/install.sh | bash -s -- -y -p 8678 -d /var/lib/fan-files -r /vol1/1000 --extra-root "存储空间2=/vol2/1000"${gl_bai}"
