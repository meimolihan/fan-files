...
const copyRealPaths = async (paths: string[]) => {
  try {
    const realPaths = await Promise.all(
      paths.map((path) => api.getAbsolutePath(path).then((r) => r.path))
    );
    copyToClipboardWithFallback(realPaths.join("\n"));
  } catch (e) {
    $showError(e as Error);
  }
};

const copyPathItem = (path: string) => {
  copyRealPaths([path]);
};
...
const copyRealPath = () => {
  ...
  if (fileStore.selectedCount > 0) {
    paths = fileStore.req.items
      .filter((_, index) => fileStore.selected.indexOf(index) !== -1)
      .map((item) => item.path);
  } else {
    paths = [removePrefix(route.path)];
  }
